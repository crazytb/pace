...
## 실험 결과 (results/step9/fig13/, 50k슬롯 × 3 seeds, SNR=14dB)

### mean_access_delay vs N (주요 방법)

| N | q0_arq | q2_arq | adap_arq | q0_harq | q2_harq | adap_harq |
|---|--------|--------|----------|---------|---------|-----------|
| 5  | 105 | 108 | 105 | 100 | 99  | 100 |
| 10 | 216 | 209 | 209 | 202 | 200 | 207 |
| 20 | 351 | 436 | 373 | 357 | 431 | 381 |
| 30 | 404 | 667 | 440 | 415 | 657 | 445 |
| 50 | 425 |1005 | 452 | 429 |1003 | 453 |

### 핵심 발견

**비단조 qsrc-delay 관계 확인**:
- N=5: q0 최적 (PHY 실패 지배 → backoff 짧을수록 유리)
- N=20: q0(351) < q1(420) < q2(436) — q0이 여전히 최적
- N=30~50: **q0이 최단** (425 vs q2 1005 at N=50) — 충돌이 많아도 q0 retry가 빠름

**예측과 다른 결과**: 예상은 "N=50에서 q2가 q0보다 지연 작음"이었으나 실제로는 q0<q2 for all N.
이유: 고충돌 환경(N=50)에서 q2(CW=63)는 첫 backoff가 길어 총 대기시간이 q0보다 큼. 
throughput은 q2>q0이지만 delay는 q0<q2. qsrc*(throughput) ≠ qsrc*(delay) 결론.

**adaptive_arq**: 모든 N에서 q0과 유사 지연 — 낮은 N에서 q0으로 수렴, 충돌 크면 소폭 상승.

**HARQ 지연 감소**: 전 조건에서 ARQ대비 HARQ가 지연 5~10% 감소 (SNR=14dB, p=0.5 조건).
HARQ 이득은 충돌률 낮은 구간(small N 또는 high qsrc)에서 상대적으로 큼.
...
| 2026-06-03 | 실험 결과 추가 — 비단조 qsrc-delay, qsrc*(delay)≠qsrc*(TP) 확인 |
