...
\begin{equation}
    ...
    \qquad
    h(x) \;=\; p_\mathrm{I}(x)-p_\mathrm{C}(x),
    \label{eq:decomp}
\end{equation}
...
The pull has a single balance point. Because solo-copy keeps the population near a common value, we read $x$ as that common operating point. A larger $x$ then means a busier channel, so $p_\mathrm{I}$ falls and $p_\mathrm{C}$ rises with $x$, and $h$ crosses zero exactly once, from above. At the crossing $x^*$ an idle slot and a collision are equally likely, and because the crossing is downward the drift restores the state from either side, raising the rate on a channel that has gone quiet and lowering it on one that has grown collision-prone. We write the restoring rate as $\kappa=-h'(x^*)>0$.

Two clarifications delimit what $x^*$ is. It is the equilibrium of the idle and collision feedback, not the fair-share target itself; we do not require $x^*=\ln\tau^*(t)$, only that the feedback settles in that target's neighborhood, which is what Phase~(d) is built to arrange. And since $\tau^*(t)$ drifts upward as the viable set drains, the analysis freezes the target over the stretch of the visit it examines, a quasi-static approximation that suffices because the question here is how the step should scale with the number of updates, not where the operating point sits exactly.

Near $x^*$ the drift linearizes. Taylor expansion gives $h(X_k)=h(x^*)+h'(x^*)\,(X_k-x^*)+O\bigl((X_k-x^*)^2\bigr)$, the constant term vanishes at the equilibrium, and dropping the quadratic remainder, the only approximation made so far, turns Eq.~\eqref{eq:decomp} into a linear recursion for the error $Y_k=X_k-x^*$,
\begin{equation}
    Y_{k+1} \;\approx\; (1-\kappa\epsilon)\,Y_k + \epsilon\,\xi_{k+1}.
    \label{eq:linrec}
\end{equation}
Each update contracts the error by the factor $1-\kappa\epsilon$ and injects a kick of size $\epsilon\xi$. At the equilibrium $h(x^*)=0$, so the noise there is the outcome itself, $\xi_{k+1}=H_k$, which is $\pm1$ whenever the observed slot is idle or collided and $0$ otherwise, giving
\begin{equation*}
    \nu^2 \;=\; \operatorname{Var}\bigl[H_k \mid X_k=x^*\bigr]
    \;=\; p_\mathrm{I}(x^*)+p_\mathrm{C}(x^*),
\end{equation*}
the fraction of observed outcomes that move the state at all.
...
\end{equation}
whose two terms accumulate differently over the visit. The first is the initial offset $Y_0=X_0-x^*$ decaying geometrically; summed over $k=0,\dots,K-1$ it contributes about $Y_0^2/(1-a^2)\approx Y_0^2/(2\kappa\epsilon)$ for small $\epsilon$, so a small step pays for its slow ramp at rate $1/\epsilon$. The second is the injected noise, which saturates at the stationary variance $\epsilon^2\nu^2/(1-a^2)\approx\epsilon\nu^2/(2\kappa)$ and is then paid on every one of the $K$ updates, so a large step pays for its jitter at rate $K\epsilon$. Multiplying by the curvature factor $q/2$,
\begin{equation}
    R_K(\epsilon) \;\approx\; \frac{q}{4\kappa}
    \left( \frac{Y_0^2}{\epsilon} + K \nu^2 \epsilon \right),
    \label{eq:regret}
\end{equation}
where the first term is the transient cost of the ramp and the second the accumulated fluctuation around the operating point. The two scale oppositely in $\epsilon$, and balancing them yields
\begin{equation}
    \epsilon^* \;=\; \frac{|Y_0|}{\nu\sqrt{K}}.
    \label{eq:eps_opt}
\end{equation}
A short visit offers few corrections, so each must be large; a long one offers many, so a smaller step that trades ramp speed for composure around the target comes out ahead.
...
\begin{figure*}[t]
    ...
    $200$ and $1680$ slots, $1.8$ and $15.1$~ms, $\Nvis=20$, $\Nnat=10$).
    ...
    slightly earlier than under basic access.}
