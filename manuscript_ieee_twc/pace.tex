...
Adaptation factor $c$ (PACE-static) & $1.5$ \\
Adaptation factor $c$ (PACE-dynamic) & $e^{C/\sqrt{W_\mathrm{eff}}}$, $C=10.16$ \\
...
\begin{figure}[t]
    ...
    visitors), PACE-static, and the fair-share reference (FS), in the mixed
...
$\tau^*(t)$ with per-slot genie knowledge of the viable set, which
...
by no one. Near-parity with the genie at every weight is therefore the
...
\begin{figure*}[t]
    ...
    PACE-static, PACE-dynamic, and FS, with $\Nvis=20$ visitor STAs and
...
both variants appear, PACE-dynamic recomputing its factor per visit from
...
ends of the sweep. PACE-static declines from $0.61$ at $0.9$~ms to $0.50$ at
...
under-serving its own visitors. PACE-dynamic compresses the swing to
...
\begin{figure}[t]
    ...
    $W_\mathrm{eff}=420$ slots, $3.78$~ms), under basic access. The
    analytic FS target
...
behavior of each scheme, which Fig.~\ref{fig:tracking} reports. Since
...
slot, against the analytic FS target. The target $\tau^*(t)$ stays near $1/(\Nvis{+}\Nnat)=1/30$
...
\begin{figure*}[t]
    ...
    $\Nnat=10$), for both access modes.
    PACE-static keeps the fixed factor $c=1.5$ at every window, while
    PACE-dynamic recomputes it per visit, $c=2.05$ at the short window
...
appears away from the reference window, which
Fig.~\ref{fig:tracking_windows} redraws at a short and a long visit. In
...
\begin{table}[t]
\centering
\caption{Per-Attempt Outcome of Visitor Transmissions\\in the Setting of Fig.~\ref{fig:tracking}, Steady State}
\label{tab:attempts}
\begin{tabular}{llccc}
\hline
...
Section~\ref{subsec:eval_duration}. The runs here use PACE-static.
...
total useful airtime within $3\%$ of a genie-aided fair-share
...
standard procedure requires. Across $0.9$--$15.1$~ms visits the
window-scaled PACE-dynamic preserves the proportionality bound where
a fixed factor drifts past it, and an ablation confirmed that the
initialization and self-exclusion are both necessary. Future work
