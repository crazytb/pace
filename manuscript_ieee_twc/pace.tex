...
The viable population fixes the operating point that any contention rule on this channel should seek. In a slot with $|\mathcal{V}(t)|$ viable STAs each attempting independently with a common probability, a success, meaning exactly one transmitter, is most likely when that probability is
\begin{equation}
    \tau^*(t) \;=\; \frac{1}{|\mathcal{V}(t)|},
    \label{eq:tau_opt}
\end{equation}
the classical slotted-ALOHA optimum applied to the viable set. Because $\mathcal{V}(t)$ is non-increasing, this target rises toward the deadline, and tracking it without knowing $|\mathcal{V}(t)|$ is what Section~\ref{sec:pace_algorithm} constructs.
