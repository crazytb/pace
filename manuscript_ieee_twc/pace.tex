...
A visit is scored by what it accumulates rather than by where it ends, since the airtime and fairness measures of Section~\ref{sec:performance_evaluation} are collected over the whole window. For a utility that is locally quadratic around $x^*$ with curvature $q>0$, summing the second moment of Eq.~\eqref{eq:linrec} over the $K$ update opportunities of a visit gives the loss
\begin{equation}
...
itself. A high success ratio alone could be achieved by transmitting
