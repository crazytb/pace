...
**파라미터**: c_coll = c_idle = **1.5** (`run_step9_fig17.py:66-67`, 2026-09-01 재조정 — PND 원값. 상세는 위 ⚠️⚠️ 블록)
...
> **⚠️⚠️ MIMD 계수 재조정 (2026-09-01): `c_coll = c_idle` 1.2 → 1.5.**
> `1.5`는 **PND 원논문 값**이고 `1.2`는 fig18 예비 스윕이 고른 값인데, 그 격자가
> `W_eff` 50~200 · **visitor 전용**이라 원고 평가 격자(`W_eff` 100~1680 · `N_nat`=10)와
> 다르다. 원고 격자에서 재튜닝(`harq_sim/tune_c.py`, 8시나리오 × 2 access × 11계수)한 결과:
> **총 airtime만 보면 1.2가 최적**(`T/T_oracle` 중앙값 1.037, 1.5는 1.008)이나
> **`ρ`가 0.358에 그친다**. 1.5는 총 airtime **2.0%**를 내주고 `ρ`를 **0.635**로 올린다.
> CLAUDE.md의 목적 서술("fairness를 최대한 보장하면서 airtime을 높인다")에 따라 **1.5 채택**.
>
> **새 수치** (`N_nat`=10, `W_eff`=420, `T` = 유용 airtime ÷ `W_eff`):
>
> | access | `N_vis` | `T` 표준 | `T` PACE | `T` FS | PACE/표준 | PACE/FS | `ρ` PACE |
> |---|---|---|---|---|---|---|---|
> | basic | 5 | 0.531 | 0.620 | 0.603 | 1.17× | 1.028 | 0.599 |
> | basic | 50 | 0.142 | 0.555 | 0.488 | **3.90×** | 1.137 | 0.614 |
> | rts | 5 | 0.698 | 0.719 | 0.733 | 1.03× | 0.981 | 0.734 |
> | rts | 50 | 0.448 | 0.713 | 0.715 | **1.59×** | 0.997 | 0.674 |
>
> 구 수치(1.2) 대비: 기준창 rts +63% → **+59%**, basic 4.3× → **3.90×** (구조 유지, 소폭 감소).
> **가장 큰 변화는 `ρ`**: 0.21~0.50 → **0.54~0.73**. visitor가 비례 몫의 절반 이하에서
> 2/3 수준으로 올라온다. `ρ` 서술을 "의도적 sub-proportional"에서 **"비례에 근접하되
> 여전히 아래"**로 완화해야 한다.
>
> **Table III 변화**: PACE 시도당 성공 basic 54%→**44%**, rts —→**40%**로 FS(40~41%)와
> 사실상 동일해진다. 구 서술("PACE가 FS보다 높은 수율")은 폐기하고 **"PACE가 FS 목표선을
> 실제로 추종한다"**로 바꾼다. ⚠️ Standard 행이 원고(14%/7.1)와 다르다(**16%/6.5**) —
> 원본 생성기가 저장소에 없어 `harq_sim/run_table3.py`로 새로 작성했고 정의가 완전히
> 일치하지 않는다.
>
> **함께 고친 버그**: `run_step9_fig26.py`가 `τ₀=1/N_visitor` + warm carry를 쓰고 있었다.
> 원고 본문(Table II, §V)과 fig27·28·29는 `1/W_eff` 매 방문 리셋이다. **fig26만 다른
> 알고리즘을 돌리고 있었고**, 계수와 무관하게 필요한 수정이었다.
>
> **미반영**: `pace.tex` 486행(Table II)과 494행(§V)에 `1.2`가 하드코딩돼 있다.
> 새 figure `fig6-1`/`fig6-2`(`N_vis` vs `J`, α=0/0.25/0.5/1.0)도 아직 본문에 없다.
>
> 재생성 완료: fig1-1_total, fig1-2_total, fig2-1, fig2-2, fig3-1, fig3-2, fig4-1,
> fig4-2, fig5-1, fig5-2, **fig6-1, fig6-2**, Table III.

> **⚠️ Native CW_min=16 수정 (2026-07-31)**: native STA의 초기/리셋 CW가 CW₀=N_total(15~60, fig24 잔재)이었던 것을 표준 CW_min=16으로 교체 (엔진·fig28·attempt table), fig26/27/28/29·Table III 전부 재생성. Native가 공격적으로 변해 절대 수치 하락·구성 변화, **상대 구조는 유지·일부 강화**: 기준창 rts +63%/basic 4.3× 유지, PACE≥FS(basic)·FS−3%(rts), duration 격차 확대(rts 11~22% below). **큰 변화 = fairness**: FS ρ 1.6→0.95(N50에서 비례선 아래), 표준 ρ 0.9~1.0(비례 근접 — 구 "over-proportional" 서사 폐기), PACE ρ 0.21→0.46/0.24→0.50(더 보수적, visitor 몫 N50 0.23). Table III: 표준 8.8/20.4 att·13~14% 성공·86~87% 충돌, PACE 1.9/2.4 att·53~54% 성공 → **yield 4×** (3×에서 복귀). PACE 방문 내 rate는 target 아래서 접근(공격적 native가 τ 램프 억제). Subsec B 서사 재작성: "표준은 붕괴한 채널의 비례 몫, PACE는 의도적 sub-proportional".

> **⚠️ Saturated traffic 전환 (2026-07-31): one-shot → saturated.** D1.2는 방문당 단일 PPDU를 규정하지 않음 (37.18.4 rule 4 = EDCA TXOP 반복 획득 허용) → 표준 부합 위해 전 시뮬 saturated로 전환: 성공 시 새 프레임 U{25..100} 즉시 샘플링·재경쟁, PACE 승자는 τ 유지, DCF 승자는 CW←CW_min 리셋, native 동일. 이탈 없음 → 𝒱(t)는 self-exclusion으로만 축소. 엔진 `_run_visit25`·fig28 visit_trace·attempt table 전부 수정, fig26/27/28/29 재생성. **결과 거의 불변** (PACE 우위 전 구간 보존): 기준창 rts +63%→**+62%**, basic 4.5×→**4.3×**; 장창(15ms)에서 표준의 one-shot 회복(경쟁자 소진 덕)이 소멸해 격차 오히려 확대 (rts +6%→+10%, basic 1.58×→1.78×); Table III: pace 3.5/4.0 att, 58~59% 성공(구 60~61), 표준 9.4/18.9, 19~21%. ρ: pace 0.19→0.88 basic / 0.22→0.90 rts. Algorithm 1의 "solo success→break" 삭제 (다음 프레임 dequeue 후 계속), §IV-C 승자 이탈 서사 재작성, III-C에 saturation 가정 1문장 명시. one-shot 수치는 아래 구 기록 잔존.
