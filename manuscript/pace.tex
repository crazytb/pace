...
Fig.~\ref{fig:topology_and_channel} depicts the two-BSS topology and channel assignment assumed throughout this work. The \emph{target BSS}, whose NPCA operation is our focus, comprises an Access Point (AP) and a set $\mathcal{N}=\{1,2,\dots,N\}$ of associated non-AP stations and they can be considered as visitors to the neighboring BSS. 

The visitors normally contend on their \emph{BSS primary channel}, Channel~\#1 in Fig.~\ref{fig:topology_and_channel}, which is the channel under observation. As the \emph{OBSS} row of the table in Fig.~\ref{fig:topology_and_channel} indicates, Channel~\#1 is also used by an overlapping BSS (OBSS) operating on the same channel; its transmissions intermittently render Channel~\#1 busy, and NPCA is invoked during these intervals. The AP designates a separate channel, Channel~\#2, as the \emph{NPCA primary channel}, to which the visitors migrate during NPCA operation. We use the qualifier ``BSS'' to distinguish the original primary channel from the NPCA primary channel, consistent with P802.11bn Draft terminology~\cite{ieee80211bn}.

Crucially, the NPCA primary channel is not unused spectrum reserved for the
target BSS: as the table in Fig.~\ref{fig:topology_and_channel} shows,
Channel~\#2 is the \emph{BSS primary channel of a neighboring BSS}. The
neighboring BSS comprises its own AP and a set $\mathcal{M}=\{1,2,\dots,M\}$
of \emph{native stations} that contend on this channel with standard
DCF/EDCA at all times. The visitors access it only opportunistically: when
an OBSS transmission occupies the target BSS's BSS primary channel
(Channel~\#1), the visitors migrate to the NPCA primary channel
(Channel~\#2) and contend there \emph{alongside the native stations} for the
duration of the OBSS busy period, before switching back. Two contention
disciplines thus coexist on the NPCA primary channel during each NPCA visit:
the native stations' persistent DCF and the visitors' bounded opportunistic
access.
...
and resumes on the BSS primary channel. Under the \emph{NPCA baseline} of
...
proposed \emph{PACE} scheme of Fig.~\ref{fig:transition_pace}, each station
...
\emph{time-varying}: the bounded window $W_\mathrm{eff}$ and the shrinking
...
contention window and \emph{halves} the attempt rate, whereas the optimal
policy calls for an \emph{increase} as stations drop out. Worse, the doubled
