...
Fig.~\ref{fig:topology_and_channel} depicts the two-BSS topology and channel assignment assumed throughout this work. The target BSS, whose NPCA operation is our focus, comprises an access point (AP) and a set $\mathcal{N}=\{1,2,\dots,\Nvis\}$ of associated non-AP STAs and they can be considered as \emph{visitors} to the neighboring BSS. The visitors normally contend on their BSS primary channel, Channel~\#1 in Fig.~\ref{fig:topology_and_channel}, which is the main channel used by the target BSS for its own transmissions.
...
Every viable STA that listened in slot $t$ updates its probability from the slot outcome $|\mathcal{D}(t)|$ using the multiplicative-increase and multiplicative-decrease (MIMD) rule inherited from neighbor discovery as
\begin{equation}
\tau_i(t') =
\begin{cases}
...
PACE instead holds the total airtime essentially flat at $0.60$--$0.63$ across the entire range, above FS at every point, reaching $4.3\times$ the standard procedure at $\Nvis=50$. The composition behind this flat total is the fair-sharing behavior by design. At $\Nvis=5$ the deliberately timid start $\tau_0=1/W_\mathrm{eff}$ leaves the channel largely to the natives, with the visitors taking only $0.04$ of the window, and as the visitor population, and hence its legitimate demand, grows, the idle-driven increase raises the visitors' share monotonically to $0.23$ at $\Nvis=50$, while self-exclusion returns slots that can no longer carry a full frame. PACE exceeds FS in total airtime here because FS enforces the per-STA fair attempt rate at the cost of collisions that basic access charges at full frame length, whereas PACE's conservative ramp avoids them.

Under RTS/CTS-protected access, shown in Fig.~\ref{fig:why_pace_rts},
a collision costs the constant $L_\mathrm{col}$ of Eq.~\eqref{eq:coll_rts},
which is $106~\mu$s, instead of a full frame, at the price of the
handshake overhead $L_\mathrm{hs}$ of $88~\mu$s on every success. Collisions being cheap, the
fair-share policy is now also airtime-efficient, and PACE tracks it within
$3\%$ over the whole range, holding $0.72$--$0.73$, while the channel under
standard visitors declines from $0.70$ to $0.44$. The handshake caps what
...
FS itself sits well above unity at low visitor demand, with $\rho=1.6$ at $\Nvis=5$, and falls to about $0.95$ at $\Nvis=50$, which calibrates the reference. Even a per-STA fair
...
proportionality is thus not attainable by any attempt-fair policy at low
demand, and the FS curve marks the practically fair operating line.

Against this reference the two schemes separate cleanly. The
standard-compliant visitors track proportionality closely,
with $\rho\approx 0.9$--$1.0$, but this is a proportional share of a
channel whose total useful airtime, as Fig.~\ref{fig:why_pace_basic}
showed, their collisions have already depressed at large $\Nvis$. PACE instead operates \emph{below} unity across
the entire range and approaches proportionality from beneath as the
visitor demand grows, with $\rho$ rising from $0.21$ to $0.46$ under basic
access and from $0.24$ to $0.50$ under RTS/CTS. This is the fairness counterpart of
...
but it does not close as the window grows, since the backoff pathologies
renew themselves under saturation, where every completed exchange
re-enters contention with a reset contention window. At short durations
PACE's own visitors contribute modestly, taking $0.14$ of the window
versus $0.29$ for the standard visitors, the one-probe budget
...
independent ones, $86$--$87\%$ of standard-visitor transmissions
...
\hline
basic   & Standard NPCA & $8.8$  & $14\%$ & $86\%$ \\
        & PACE          & $1.9$  & $54\%$ & $46\%$ \\
        & FS            & $6.9$  & $40\%$ & $60\%$ \\
\hline
RTS/CTS & Standard NPCA & $20.4$ & $13\%$ & $87\%$ \\
        & PACE          & $2.4$  & $53\%$ & $47\%$ \\
        & FS            & $9.6$  & $40\%$ & $60\%$ \\
\hline
\end{tabular}
...
spends $8.8$ attempts per visit (and $20.4$ under RTS/CTS, where cheap
collisions let it retry faster) yet only $13$--$14\%$ of them succeed;
the rest are synchronized collisions. PACE makes far fewer attempts,
$1.9$ and $2.4$ respectively, and lands $53$--$54\%$ of
...
access the total airtime collapses to $0.15$--$0.03$, an order of
...
window has drained. Protection does not rescue it, with totals of $0.40$--$0.12$ that
remain below the standard baseline throughout, because the failure is not
...
at $\Nvis=5$ under RTS/CTS the aggressive visitors seize $0.37$ of the
channel while the ten natives retain $0.03$. A probabilistic access
...
grows with the visitor population. Total airtime drops by about $1\%$
at $\Nvis=5$ and by $5\%$ at $\Nvis=50$ under both access modes,
falling from $0.603$ to $0.575$ under basic access and from $0.724$ to
$0.691$ under RTS/CTS at $\Nvis=50$. Without Phase~(b), a visitor whose frame no
...
within $3\%$ of the fair-share reference across $5$--$50$ visitors under
RTS/CTS-protected access, up to $63\%$ above the standard
procedure, and under basic access held the channel where the standard
procedure collapsed, reaching a $4.3\times$ advantage. The gain is not
...
hold a near-proportional share of a channel their collisions have
...
a fraction of the baseline's attempts with a $4\times$ higher
