...
\begin{abstract}
Modern Wi-Fi deployments increasingly suffer from spectrum underutilization. While an overlapping basic service set (OBSS) transmission occupies the primary channel, adjacent non-primary channels often sit idle since the conventional IEEE 802.11 mandates that the primary channel be included in every channel access. 
Hence, IEEE 802.11bn introduces non-primary channel access (NPCA) to exploit these idle intervals. The standard contention mechanism, however, is not well suited to NPCA's constrained transmission windows, as its collision-triggered backoff growth synchronizes stations into repeated collisions or pushes their counters beyond the remaining opportunity, wasting time that could otherwise carry data.
We propose PACE, a probabilistic adaptive contention control scheme that enables stations to collectively track the time-varying fair-share transmission probability of the depleting window without central coordination. PACE combines a deadline-scaled initialization and a distributed multiplicative feedback rule---copying the successful sender's probability on solo receptions---with a prospective self-exclusion mechanism that silences stations whose frame length exceeds the remaining window.
We evaluate PACE in IEEE 802.11 standard timing on a non-primary channel shared with native stations, against the standard-compliant NPCA baseline and a genie-aided fair-share reference. Across $5$--$50$ visiting stations, PACE keeps the total useful airtime of the shared channel within $2\%$ of the fair-share reference under RTS/CTS-protected access---up to $63\%$ above the standard procedure, and up to $4.5\times$ under basic access---while holding the visitors' airtime at or below their population-proportional share. The advantage holds across visit durations from one to fifteen milliseconds and grows with the visitor population, where the standard backoff idles behind frozen counters and collides in synchronized bursts; PACE delivers this with a fraction of the baseline's transmission attempts and a three-fold higher per-attempt success ratio. An ablation study confirms that both the deadline-scaled initialization and the self-exclusion rule are necessary for these gains.
These results establish probabilistic, deadline-aware contention as a practical design reference for non-primary channel access in next-generation Wi-Fi networks.
\end{abstract}
...
carry frames of varying lengths, the problem is further compounded: the
draft leaves unspecified what a STA does when its frame no longer fits the
remaining window, so a compliant STA may keep contending---or even
transmit and be cut off at timer expiry---despite having no viable
...
NPCA's bounded-window structure: \textit{(i)} the fair-share transmission
...
($\mathrm{CW}_{\min}=16$, binary exponential backoff) as Draft~1.2
...
visit inverts the premise of binary exponential backoff: as the window
