"""
Step 3 시뮬레이션 실행 스크립트 — HARQ-CC Chase Combining

출력: results/step3/sim_trace.csv
       results/step3/summary.txt

사용법:
  python harq_sim/run_step3.py [OPTIONS]
  ...
  --no-harq        : HARQ 비활성화 → ARQ-only (Step 2 비교용)
  --out-dir  PATH  : 출력 디렉토리 (기본 results/step3)
"""
...
def print_summary(sim: Simulator, args: argparse.Namespace) -> str:
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("Step 3 Simulation Summary — HARQ-CC Chase Combining")
    lines.append("=" * 80)
    ...
    lines.append(
        ...
        f"{'PHY_err':>8} {'HARQ_ok':>8} {'HARQ_fail':>9} {'PDR':>6} {'Col_p':>7} {'N_trans':>8}"
    )
    lines.append("-" * 90)
...
def main():
    parser = argparse.ArgumentParser(description="HARQ-NPCA Step 3 — HARQ-CC simulator")
    ...
    parser.add_argument("--out-dir",      type=str,   default="results/step3")
