"""
Step 7 시뮬레이션 실행 스크립트 — LLM Reward Designer

출력: results/step7/sim_trace.csv
       results/step7/summary.txt
       results/step7/llm_profile.json

사용법:
  python harq_sim/run_step7.py [OPTIONS]
  ...
  --intent        S  : 자연어 operator intent (기본 "balanced throughput and latency")
  --mock             : LLM API 미사용 (mock 키워드 매칭, 기본)
  --no-mock          : 실제 Anthropic API 사용 (ANTHROPIC_API_KEY 필요)
  --llm-interval  N  : LLM 재호출 에피소드 주기 (기록 전용, 기본 50)
  --out-dir      PATH : 출력 디렉토리 (기본 results/step7)
"""
...
def print_summary(
    ...
    lines = []
    lines.append("=" * 110)
    lines.append("Step 7 Simulation Summary — LLM Reward Designer")
    lines.append("=" * 110)
    ...
    lines.append(f"  LLM mock      : {not args.no_mock}")
    lines.append(f"  LLM interval  : every {args.llm_interval} episodes")
    lines.append("")

    lines.append("LLM-Designed Reward Profile:")
    lines.append(f"  Intent name   : {profile['intent_name']}")
    lines.append("  Weights:")
    for k, v in profile["weights"].items():
        lines.append(f"    {k:<20}: {v:.3f}")
    lines.append("  Constraints:")
    for k, v in profile.get("constraints", {}).items():
    ...
    lines.append("")
    lines.append(f"  Reward ({profile['intent_name']}): {reward:.6f}")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="HARQ-NPCA Step 7 — LLM Reward Designer")
    ...
    parser.add_argument("--llm-interval",   type=int,   default=50,
                        help="LLM re-call interval (episodes, informational)")
    parser.add_argument("--out-dir",        type=str,   default="results/step7")
    args = parser.parse_args()

    # ── LLM reward design ─────────────────────────────────────────────────────
    use_mock = not args.no_mock
    designer = LLMRewardDesigner(use_mock=use_mock)
    print(f"[LLM] Designing reward profile for intent: \"{args.intent}\" "
          f"({'mock' if use_mock else 'API'})")
    profile = designer.design_reward(args.intent)
    print(f"[LLM] Profile: {profile['intent_name']} — "
          f"weights={list(profile['weights'].values())}")
    ...
    summary     = print_summary(sim, args, profile)
